plot_cna <- function(binned_gr, chrom_sizes_gr,
  ...
  df <- as.data.frame(binned_gr) %>%
    dplyr::inner_join(as.data.frame(chrom_sizes_gr),
      by = "seqnames",
...
.annotate_cnv <- function(gg,
  ...
  cnv_df <- as.data.frame(cnv_gr) %>%
    ...
    dplyr::left_join(as.data.frame(chrom_sizes_gr),
      by = "seqnames",
      suffix = c("_cnv", "_chrom_sizes")
    ) %>%
    dplyr::mutate(
      global_start = start_cnv + cum_start,
  ...
  gg <- gg +
    geom_rect(
      data = cnv_df,
      aes(
        xmin = global_start, xmax = global_end,
